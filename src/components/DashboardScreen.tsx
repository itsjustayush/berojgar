    ...
    <div className="page-reveal mx-auto w-full max-w-[1440px] px-5 pb-16 pt-10 sm:px-8 lg:px-12 lg:pt-16">
      <section className="grid gap-10 lg:grid-cols-[1.15fr_.85fr] lg:items-end lg:gap-16">
        <div className="max-w-3xl">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#EF4E22]/20 bg-[#EF4E22]/[.08] px-3 py-2 font-mono text-[10px] uppercase tracking-[.2em] text-[#EF4E22]">
          ...
          <p className="mt-7 max-w-xl text-base leading-7 text-white/55 sm:text-lg">Berozgar Vault gives teams a temporary place to talk and move files together. No profile graph, no archive, no friction between a room and the people in it.</p>
          <div className="mt-8 flex flex-wrap items-center gap-4 font-mono text-[10px] uppercase tracking-[.16em] text-white/45">
            <span className="inline-flex items-center gap-2"><Radio size={14} className="text-[#EF4E22]" /> Live transport</span>
            <span className="inline-flex items-center gap-2"><LockKeyhole size={14} className="text-[#EF4E22]" /> Memory-first</span>
            <span className="inline-flex items-center gap-2"><Users size={14} className="text-[#EF4E22]" /> Guest access</span>
          </div>
        </div>

        <div className="glass-panel relative overflow-hidden rounded-[2rem] p-5 sm:p-7">
          <div className="absolute -right-20 -top-20 h-40 w-40 rounded-full bg-[#EF4E22]/10 blur-3xl" />
          ...
          {editingNick && (
            <form onSubmit={handleSaveNick} className="relative mt-4 flex gap-2">
              ...
              <input id="dashboard-nickname" value={nickVal} onChange={(event) => setNickVal(event.target.value)} maxLength={18} className="min-w-0 flex-1 rounded-xl border border-white/15 bg-black/30 px-3 py-3 text-sm text-white outline-none focus:border-[#EF4E22]" />
              <button type="submit" className="grid h-11 w-11 place-items-center rounded-xl bg-[#EF4E22] text-[#FFF9F3]" aria-label="Save nickname"><Check size={17} /></button>
            </form>
          )}
      ...
      <section className="mt-14 grid gap-4 lg:grid-cols-2 lg:gap-5">
        <article className="stagger-reveal group relative flex min-h-[360px] flex-col justify-between overflow-hidden rounded-[2rem] border border-[#EF4E22]/35 bg-[#EF4E22] p-6 text-[#FFF9F3] shadow-[0_0_50px_rgba(239,78,34,.12)] sm:p-8" style={{ '--delay': '120ms' } as CSSProperties}>
          <div className="absolute -bottom-24 -right-16 h-64 w-64 rounded-full border-[32px] border-white/[.08] transition-transform duration-500 group-hover:scale-110" />
          <div className="relative"><div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-[.18em] opacity-75"><span>01 / Create</span><Plus size={18} /></div><h2 className="mt-12 max-w-sm text-4xl font-semibold leading-[.92] tracking-[-.07em] sm:text-5xl">Start a room<br /><em className="font-display font-normal not-italic">in seconds.</em></h2><p className="mt-5 max-w-sm text-sm leading-6 text-white/80">Generate a short-lived room and bring people in with a code or link. The host controls the room’s first connection.</p></div>
          <button onClick={onCreateRoom} className="relative mt-8 inline-flex w-full items-center justify-between rounded-full bg-[#0b1326] px-5 py-4 text-sm font-medium text-white transition-transform duration-200 hover:scale-[1.015] active:scale-[.98]">Create private room <ArrowRight size={18} /></button>
        </article>
      ...
      <section className="mt-8 grid gap-3 border-t border-white/10 pt-7 sm:grid-cols-3">
        {guarantees.map((guarantee, index) => <div key={guarantee} className="stagger-reveal flex items-start gap-3 text-sm text-white/50" style={{ '--delay': `${280 + index * 70}ms` } as CSSProperties}><span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full border border-[#EF4E22]/35 text-[#EF4E22]"><Check size={11} /></span>{guarantee}</div>)}
      </section>

      <div className="mt-8 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.18em] text-white/30"><ShieldCheck size={14} className="text-[#EF4E22]" /> We do not retain room content. Signaling remains transient while peers are online.</div>
    </div>
  );
}
